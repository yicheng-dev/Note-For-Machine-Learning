			...
			\section{集成学习}
				...
				\subsection{Boosting}
					\begin{itemize}
						...
						\item AdaBoost算法基于``加性模型$H(\bm{x})=\sum_{t=1}^{T}\alpha_th_t(\bm{x})$，最小化指数损失函数\[\ell_{\mathrm{exp}}(H\mid\mathcal{D})=\mathbb{E}_{\bm{x}\sim\mathcal{D}}[e^{-f(\bm{x})H(\bm{x})}]\]\begin{algorithm}
							\caption{AdaBoost算法}
							\begin{algorithmic}[1]
