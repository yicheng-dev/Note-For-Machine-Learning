...
\begin{document}
			...
			\section{聚类}
				...
				\subsection{原型聚类}
					...
					\subsubsection{学习向量量化}
						...
						\end{itemize}
					\subsubsection{高斯混合聚类}
						\begin{itemize}
							\item 高斯分布、EM算法、极大似然法和聚类的结合
							\item 服从高斯分布的概率密度函数\[p(\bm{x\mid \bm{\mu},\bm{\Sigma}})=\frac{1}{(2\pi)^{\frac{n}{2}}|\bm{\Sigma}|^{\frac{1}{2}}}e^{-\frac{1}{2}(\bm{x}-\bm{\mu})^T\bm{\Sigma}^{-1}(\bm{x}-\bm{\mu})}\]
							\item 高斯混合分布\[p_{\mathcal{M}}(\bm{x})=\sum_{i=1}^{k}\alpha_i\cdot p(\bm{x}\mid\bm{\mu}_i,\bm{\Sigma}_i)\]
							\item 生成样本$\bm{x}_j$的每一个高斯混合成分的后验分布（记为$\gamma_{ji}$）\[p_{\mathcal{M}}(z_j=i\mid\bm{x}_j)=\frac{P(z_j=i)\cdot p_{\mathcal{M}}(\bm{x}_j\mid z_j=i)}{p_{\mathcal{M}}(\bm{x}_j)}=\frac{\alpha_i\cdot p(\bm{x}_j\mid \bm{\mu}_i,\bm{\Sigma}_i)}{\sum_{l=1}^{k}\alpha_l\cdot p(\bm{x}_j\mid\bm{\mu}_l,\bm{\Sigma}_l)}\]
							\item 算法描述\begin{algorithm}
								\caption{高斯混合聚类算法}
								\begin{algorithmic}[1]
									\REQUIRE 样本集$D=\{\bm{x}_1,\bm{x}_2,\dots,\bm{x}_m\}$、高斯混合成分个数$k$
									\ENSURE 簇划分$\mathcal{C}=\{C_1,C_2,\dots,C_k\}$
									\STATE 初始化高斯混合分布的模型参数$\{(\alpha_i,\bm{\mu}_i,\bm{\Sigma}_i)\mid 1\le i\le k\}$; \\
									\REPEAT
										\FOR{$j=1,2,\dots,m$}
											\STATE 计算$\bm{x}_j$由各混合成分生成的后验概率，即$\gamma_{ji}=p_{\mathcal{M}}(z_j=i\mid\bm{x}_j)(1\le i\le k)$; \\
										\ENDFOR
										\FOR{$i=1,2,\dots,k$}
											\STATE 计算新均值向量：$\bm{\mu}_i'=\frac{\sum_{j=1}^{m}\gamma_{ji}\bm{x}_j}{\sum_{j=1}^{m}\gamma_{ji}}$; \\
											\STATE 计算新协方差矩阵：$\bm{\Sigma}_i'=\frac{\sum_{j=1}^{m}\gamma_{ji}(\bm{x}_j-\bm{\mu}_i')(\bm{x}_j-\bm{\mu}_i')^T}{\sum_{j=1}^{m}\gamma_{ji}}$; \\
											\STATE 计算新混合系数：$\alpha_i'=\frac{\sum_{j=1}^{m}\gamma_{ji}}{m}$; \\
										\ENDFOR
										\STATE 将模型参数$\{(\alpha_i,\bm{\mu}_i,\bm{\Sigma}_i)\mid 1\le i\le k\}$更新为$\{(\alpha_i',\bm{\mu}_i',\bm{\Sigma}_i')\mid 1\le i\le k\}$; \\
									\UNTIL{满足停止条件}
									\STATE $C_i=\emptyset(1\le i\le k)$; \\
									\FOR{$j=1,2,\dots,m$}
										\STATE 根据$\lambda_j=\arg\max_{i\in\{1,2,\dots,k\}}\gamma_{ji}$确定$\bm{x}_j$的簇标记$\lambda_j$; \\
										\STATE 将$\bm{x}_j$划入相应的簇：$C_{\lambda_j}=C_{\lambda_j}\cup\{\bm{x}_j\}$; \\
									\ENDFOR
								\end{algorithmic} 
							\end{algorithm}
						\end{itemize}
				\subsection{密度聚类}
					\begin{itemize}
						\item 密度聚类算法从样本密度的角度考察样本之间的可连接性，基于可连接样本不断扩展聚类簇以获得最终的聚类结果。
						\item DBSCAN算法基于一组邻域参数$(\epsilon,MinPts)$来刻画样本分布的紧密程度\begin{itemize}
							\item $\epsilon-$邻域：$N_\epsilon(\bm{x}_j)=\{\bm{x}_i\in D\mid\mathrm{dist}(\bm{x}_i,\bm{x}_j)\le\epsilon\}$
							\item 核心对象：$|N_\epsilon(\bm{x}_j)|\ge MinPts$ $\rightarrow$ $\bm{x}_j$是一个核心对象
							\item $\bm{x}_j$位于$\bm{x}_i$的$\epsilon-$邻域中 $\cap$ $\bm{x}_i$是核心对象 $\rightarrow$ $\bm{x}_j$由$\bm{x}_i$密度直达
							\item 对$\bm{x}_i$与$\bm{x}_j$若存在样本序列$\bm{p}_1,\bm{p}_2,\dots,\bm{p}_n$，其中$\bm{p}_1=\bm{x}_i,\bm{p}_n=\bm{x}_j$，且$\bm{p}_{i+1}$由$\bm{p}_i$密度直达 $\rightarrow$ $\bm{x}_j$由$\bm{x}_i$密度可达
							\item 对$\bm{x}_i$与$\bm{x}_j$，若存在$\bm{x}_k$使得$\bm{x}_i$与$\bm{x}_j$均由$\bm{x}_k$密度可达 $\rightarrow$ $\bm{x}_i$与$\bm{x}_j$密度相连
							\item 簇：由密度可达关系导出的最大的密度相连样本集合，满足``连接性''（簇中任意两个样本密度相连）和``最大性''（簇中任意一个样本密度可达的样本都在该簇中）
						\end{itemize}
						\item 算法描述\begin{algorithm}
							\caption{DBSCAN算法}
							\begin{algorithmic}[1]
								\REQUIRE 样本集$D=\{\bm{x}_1,\bm{x}_2,\dots,\bm{x}_m\}$、邻域参数$(\epsilon,MinPts)$
								\ENSURE 簇划分$\mathcal{C}=\{C_1,C_2,\dots,C_k\}$
								\STATE 初始化核心对象集合：$\Omega = \emptyset$; \\
								\FOR{$j=1,2,\dots,m$}
									\STATE 确定样本$\bm{x}_j$的$\epsilon$-邻域$N_\epsilon(\bm{x}_j)$; \\
									\IF{$|N_\epsilon(\bm{x}_j)|\ge MinPts$}
										\STATE 将样本$\bm{x}_j$加入核心对象集合：$\Omega = \Omega\cup\{\bm{x}_j\}$; \\
									\ENDIF
								\ENDFOR
								\STATE 初始化聚类簇数和未访问样本集合：$k=0,\quad \Gamma = D$;\\
								\WHILE{$\Omega\neq\emptyset$}
									\STATE 记录当前未访问样本集合：$\Gamma_{\mathrm{old}}=\Gamma$; \\
									\STATE 随机选取一个核心对象$\bm{o}\in\Omega$，初始化队列$Q=\langle\bm{o}\rangle$; \\
									\STATE $\Gamma=\Gamma\backslash\{\bm{o}\}$; \\
									\WHILE{$Q\neq\emptyset$}
										\STATE 取出队列$Q$中的某个样本$\bm{q}$; \\
										\IF{$|N_\epsilon(\bm{q})|\ge MinPts$}
											\STATE 令$\Delta = N_\epsilon(\bm{q})\cap\Gamma$; \\
				...
				\subsection{层次聚类}
					\begin{itemize}
						\item AGNES算法：自底向上层次聚类。先将每个样本视作一个初始聚类簇，再不断合并距离最近的两个聚类簇，达到预设的聚类簇个数。
						\item 聚类簇之间的距离计算\begin{align*}
							& \textrm{最小距离：}d_{\mathrm{min}}(C_i,C_j)=\min\limits_{\bm{x}\in C_i,\bm{z}\in C_j}\mathrm{dist}(\bm{x},\bm{z}) \\
							& \textrm{最大距离：}d_{\mathrm{max}}(C_i,C_j)=\max\limits_{\bm{x}\in C_i,\bm{z}\in C_j}\mathrm{dist}(\bm{x},\bm{z}) \\
							& \textrm{平均距离：}d_{\mathrm{avg}}(C_i,C_j)=\frac{1}{|C_i||C_j|}\sum_{\bm{x}\in C_i}^{}\sum_{\bm{z}\in C_j}^{}\mathrm{dist}(\bm{x},\bm{z})
						\end{align*}
						\item 算法描述\begin{algorithm}
							\caption{AGNES算法}
							\begin{algorithmic}[1]
								\REQUIRE 样本集$D=\{\bm{x}_1,\bm{x}_2,\dots,\bm{x}_m\}$、簇距离度量函数$d$、聚类簇数$k$.
								\ENSURE 簇划分$\mathcal{C}=\{C_1,C_2,\dots,C_k\}$
								\FOR{$j=1,2,\dots,m$}
									\STATE $C_j=\{\bm{x}_j\}$; \\
								\ENDFOR
								\FOR{$i=1,2,\dots,m$}
									\FOR{$j=i+1,\dots,m$}
										\STATE $M(j,i)=M(i,j)=d(C_i,C_j)$; \\
									\ENDFOR
								\ENDFOR
								\STATE 设置当前聚类簇个数：$q=m$; \\
								\WHILE{$q>k$}
									\STATE 找出距离最近的两个聚类簇$C_{i^*}$和$C_{j^*}$; \\
									\STATE 合并$C_{i^*}$和$C_{j^*}$：$C_{i^*}=C_{i^*}\cup C_{j^*}$; \\
									\FOR{$j=j^*+1, j^*+2, \dots, q$}
										\STATE 将聚类簇$C_j$重编号为$C_{j-1}$; \\
									\ENDFOR
									\STATE 删除距离矩阵$M$的第$j^*$行与第$j^*$列; \\
									\FOR{$j=1,2,\dots,q-1$}
										\STATE $M(j,i^*)=M(i^*,j)=d(C_{i^*}, C_j)$; \\
									\ENDFOR
									\STATE $q = q-1$; \\
								\ENDWHILE
							\end{algorithmic} 
			...
			\section{降维与度量学习}
				\subsection{$k$近邻学习}
					\begin{itemize}
						\item 机制：给定测试样本，基于某种距离度量找出训练集中与其最靠近的$k$个训练样本，并基于它们采用投票法或平均法进行预测。
						\item 是一种``懒惰学习''（训练时间开销为零，收到测试样本后才进行处理，相对于``急切学习''）。
						\item 最近邻分类器的泛化错误率不超过贝叶斯最优分类器的两倍。
					\end{itemize}
				\subsection{低维嵌入}
					\begin{itemize}
						\item 密采样：任意测试样本$\bm{x}$附近任意小的$\delta$距离范围内总能找到一个训练样本。若属性维数过多，满足密采样条件所需的样本过于巨大，导致数据样本稀疏、距离计算困难，引起``维数灾难''。
						\item 降维：通过某种数学变换将原始高维属性空间转变为一个低维子空间。
						\item 多维缩放（MDS）：设$m$个样本在原始空间的距离矩阵为$\mathbf{D}\in\mathbb{R}^{m\times m}$，其中第$i$行第$i$列为样本$\bm{x}_i$到$\bm{x}_j$的距离。目标是获得样本在$d'$维空间的表示$\mathbf{Z}\in\mathbb{R}^{d'\times m},d'\le d$，且任意两个样本在$d'$维空间中的欧氏距离等于原始空间中的距离，即$||\bm{z}_i-\bm{z}_j||=dist_{ij}$。\begin{algorithm}
							\caption{MDS算法}
							\begin{algorithmic}[1]
								\REQUIRE 距离矩阵$\mathbf{D}\in\mathbb{R}^{m\times m}$，其元素$dist_{ij}$为样本$\bm{x}_i$到$\bm{x}_j$的距离、低维空间维数$d'$
								\ENSURE 矩阵$\tilde{\mathbf{V}}\tilde{\bm{\Lambda}}^{1/2}\in\mathbb{R}^{m\times d'}$，每行是一个样本的低维坐标
								\STATE 计算$dist_{i\cdot}^2=\frac{1}{m}\sum_{j=1}^{m}dist_{ij}^2$及$dist_{\cdot j}^2,dist_{\cdot\cdot}^2$; \\
								\STATE 根据$b_{ij}=-\frac{1}{2}(dist_{ij}^2-dist_{i\cdot}^2-dist_{\cdot j}^2+dist(\cdot\cdot)^2)$计算矩阵$\mathbf{B}$; \\
								\STATE 对矩阵$\mathbf{B}$做特征值分解; \\
								\STATE 取$\tilde{\bm{\Lambda}}$为$d'$个最大特征值所构成的对角矩阵，$\tilde{\mathbf{V}}$为相应的特征向量矩阵; \\
							\end{algorithmic} 
						\end{algorithm}
						\item 线性降维方法：给定$d$维空间的样本$\mathbf{X}=(\bm{x}_1,\bm{x}_2,\dots,\bm{x}_m)\in\mathbb{R}^{d\times m}$，变换之后得到$d'\le d$维空间中的样本\[\mathbf{Z}=\mathbf{W}^T\mathbf{X}\]其中$\mathbf{W}\in\mathbb{R}^{d\times d'}$是变换矩阵，$\mathbf{Z}\in\mathbb{R}^{d'\times m}$是样本在新空间中的表达。
					\end{itemize}
				\subsection{主成分分析}
					\begin{itemize}
						\item 对于正交属性空间中的样本点，若存在一个超平面，满足``最近重构性''（样本点到这个超平面的距离都足够近）、``最大可分性''（样本点在这个超平面上的投影能尽可能分开），可分别从这两点推导出主成分分析。
						\item 设投影变换后得到的新坐标系为$W=(\bm{w}_1,\bm{w}_2,\dots,\bm{w}_{d'})$，则PCA的优化目标为\begin{align*}
							& \max\limits_{\mathbf{W}} \quad\mathrm{tr}(\mathbf{W}^T\mathbf{XX}^T\mathbf{W})\\
							& \mathrm{s.t.} \quad\mathbf{W}^T\mathbf{W}=\mathbf{W}
						\end{align*}
						\item 算法描述\begin{algorithm}
							\caption{PCA算法}
							\begin{algorithmic}[1]
								\REQUIRE 样本集$D=\{\bm{x}_1,\bm{x}_2,\dots,\bm{x}_m\}$、低维空间维数$d'$
								\ENSURE 投影矩阵$\mathbf{W}^*=(\bm{w}_1,\bm{w}_2,\dots,\bm{w}_{d'})$
								\STATE 对所有样本进行中心化：$\bm{x}_i\leftarrow\bm{x}_i-\frac{1}{m}\sum_{i=1}^{m}\bm{x}_i$; \\
								\STATE 计算样本的协方差矩阵：$\mathbf{X}\mathbf{X}^T$; \\
								\STATE 对协方差矩阵$\mathbf{XX}^T$做特征值分解; \\
								\STATE 取最大的$d'$个特征值所对应的特征向量$\bm{w}_1,\bm{w}_2,\dots,\bm{w}_{d'}$; \\
							\end{algorithmic} 
						\end{algorithm}
						\item $d'$值可由用户指定，也可以通过kNN+交叉验证自动取值，也可取使得$\sum_{i=1}^{d'}\lambda_i/\sum_{i=1}^{d}\lambda_i\ge t$的最小$d'$值。
						\item 降维（丢弃$d-d'$个特征值的特征向量）的意义：增大采样密度、减小噪声影响。
					\end{itemize}
				\subsection{核化线性降维}
					\begin{itemize}
						\item 核主成分分析（KPCA）：基于核技巧对PCA进行核化，实现非线性降维。
						\item KPCA的过程\begin{enumerate}
							\item 由PCA中的$\mathbf{XX}^T\bm{w}_i=\lambda_i\bm{w}_i$，设$\bm{z}_i$是样本点$\bm{x}_i$在高维特征空间的像，得$(\sum_{i=1}^{m}\bm{z}_i\bm{z}_i^T)\bm{w}_j=\lambda_j\bm{w}_j$且\[\bm{w}_i=\sum_{i=1}^{m}\bm{z}_i\frac{\bm{z}_i^T\bm{w}_j}{\lambda_j}=\sum_{i=1}^{m}\bm{z}_i\alpha_i^j\]
							\item 假设$\bm{z}_i$是由原始属性空间中的样本点$\bm{x}_i$通过映射$\phi$产生，则\[\bigg(\sum_{i=1}^{m}\phi(\bm{x}_i)\phi(\bm{x}_i)^T\bigg)\bm{w}_j=\lambda_j\bm{w}_j\]\[\bm{w}_j=\sum_{i=1}^{m}\phi(\bm{x}_i)\alpha_i^j\]
							\item 引入核函数\[\kappa(\bm{x}_i,\bm{x}_j)=\phi(\bm{x}_i)^T\phi(\bm{x}_j)\]\[\mathbf{K}\bm{\alpha}^j=\lambda_j\bm{\alpha}^j\]取$\mathbf{K}$最大的$d'$个特征值所对应的特征向量即可。
							\item 对新样本$\bm{x}$，其投影后的第$j$维坐标为\[z_j=\bm{w}_j^T\phi(\bm{x})=\sum_{i=1}^{m}\alpha_i^j\phi(\bm{x}_i)^T\phi(\bm{x})=\sum_{i=1}^{m}\alpha_i^j\kappa(\bm{x}_i,\bm{x})\]
						\end{enumerate}
					\end{itemize}
				\subsection{流形学习}
				
					流形在局部具有欧氏空间的性质，能用欧氏距离进行计算。可以在局部建立降维映射关系。
					\subsubsection{等度量映射}
						\begin{itemize}
							\item 等度量映射（Isomap）：低维流形嵌入到高维空间之后，直接在高维空间中计算直线距离具有误导性。计算``测地线''距离时，可以利用流形在局部上与欧氏空间同胚这一性质，对每个点基于欧氏距离找出近邻点，然后建立一个近邻连接图，求两点之间最短路径即可。（Dijkstra算法等）
							\item 算法描述\begin{algorithm}
								\caption{Isomap算法}
								\begin{algorithmic}[1]
									\REQUIRE 样本集$D=\{\bm{x}_1,\bm{x}_2,\dots,\bm{x}_m\}$、近邻参数$k$、低维空间维数$d'$
									\ENSURE 样本集$D$在低维空间的投影$Z=\{\bm{z}_1,\bm{z}_2,\dots,\bm{z}_m\}$
									\FOR{$i=1,2,\dots,m$}
										\STATE 确定$\bm{x}_i$的$k$近邻; \\
										\STATE $\bm{x}_i$与$k$近邻点之间的距离设置为欧氏距离，与其他点的距离设置为无穷大; \\
									\ENDFOR
									\STATE 调用最短路径算法计算任意两样本点之间的距离$\mathrm{dist}(\bm{x}_i,\bm{x}_j)$; \\
									\STATE 将$\mathrm{dist}(\bm{x}_i,\bm{x}_j)$作为MDS算法的输入; \\
									\RETURN MDS算法的输出
								\end{algorithmic} 
							\end{algorithm}
							\item 新样本映射到低维空间的权宜之计：将训练样本的高维空间坐标作为输入、低维空间坐标作为输出，训练一个回归学习器对新样本的低维空间进行预测。
						\end{itemize}
					\subsubsection{局部线性嵌入}
						\begin{itemize}
							\item 局部线性嵌入（LLE）：试图保持邻域内样本之间的线性关系。希望诸如$\bm{x}_i=w_{ij}\bm{x}_j+w_{ik}\bm{x}_k+w_{il}\bm{x}_l$在低维空间中得以保持。
							\item LLE操作过程：\begin{enumerate}
								\item 为每个样本$\bm{x}_i$找到其近邻下标集合$Q_i$，再计算基于$Q_i$中的样本点对$\bm{x}_i$进行线性重构的系数$\bm{w}_i$\begin{align*}
									& \min\limits_{\bm{w}_1,\bm{w}_2,\dots,\bm{w}_m}\quad\sum_{i=1}^{m}\bigg|\bigg|\bm{x}_i-\sum_{j\in Q_i}^{}w_{ij}\bm{x}_j\bigg|\bigg|_2^2\\
									& \mathrm{s.t.}\quad\sum_{j\in Q_i}^{}w_{ij}=1
								\end{align*}
								\item 在低维空间中保持$\bm{w}_i$不变，于是$\bm{x}_i$对应的低维空间坐标$\bm{z}_i$通过下式求解\[\min\limits_{\bm{z}_1,\bm{z}_2,\dots,\bm{z}_m}\sum_{i=1}^{m}\bigg|\bigg|\bm{z}_i-\sum_{j\in Q_i}^{}w_{ij}\bm{z}_j\bigg|\bigg|_2^2\]
								\item 令$\mathbf{Z}=(\bm{z}_1,\bm{z}_2,\dots,\bm{z}_m)\in\mathbb{R}^{d'\times m}$及$(\mathbf{W})_{ij}=w_ij$\[\mathbf{M}=(\mathbf{I}-\mathbf{W})^{\textrm{T}}(\mathbf{I}-\mathbf{W})\]于是优化目标化为\begin{align*}
									& \min\limits_{\mathbf{Z}}\quad\mathrm{tr}(\mathbf{ZMZ}^{\textrm{T}}) \\
									& \mathrm{s.t.}\quad \mathbf{ZZ}^{\textrm{T}}=\mathbf{I}
								\end{align*}
								\item 对$\mathbf{M}$进行特征值分解，最小的$d'$个特征值所对应的特征向量组成的矩阵为$\mathbf{Z}^{\textrm{T}}$。
							\end{enumerate}
							\item 算法描述\begin{algorithm}
								\caption{LLE算法}
								\begin{algorithmic}[1]
									...
									\FOR{$i=1,2,\dots,m$}
										\STATE 确定$\bm{x}_i$的$k$近邻; \\
										\STATE 求得$w_{ij},j\in Q_i$; \\
										\STATE 对于$j\notin Q_i$，令$w_{ij}=0$; \\
									\ENDFOR
									\STATE 根据$\mathbf{M}=(\mathbf{I}-\mathbf{W})^{\textrm{T}}(\mathbf{I}-\mathbf{W})$求出$\mathbf{M}$; \\
									\STATE 对$\mathbf{M}$进行特征值分解; \\
									\RETURN $\mathbf{M}$的最小$d'$个特征值对应的特征向量
								\end{algorithmic} 
							\end{algorithm}
						\end{itemize}
				\subsection{度量学习}
					\begin{itemize}
						\item 距离度量模型的推广\begin{enumerate}
							\item 若每个维度等权重，对两个$d$维样本$\bm{x}_i$和$\bm{x}_j$，平方欧氏距离为\[\mathrm{dist}_{\mathrm{ed}}^2(\bm{x}_i,\bm{x}_j)=||\bm{x}_i-\bm{x}_j||_2^2=dist_{ij,1}^2+dist_{ij,2}^2+\dots+dist_{ij,d}^2\]
							\item 若不同属性的权重不同，引入属性权重$\bm{w}$。$w_i\ge0$，$\mathbf{W}=\mathrm{diag}(\bm{w})$是一个对角矩阵，$(\mathbf{W})_{ii}=w_i$。\begin{align*}
								\mathrm{dist}_{\mathrm{wed}}^2(\bm{x}_i,\bm{x}_j) = & ||\bm{x}_i-\bm{x}_j||_2^2 \\
								= & w_1\cdot dist_{ij,1}^2+w_2\cdot dist_{ij, 2}^2 +\dots +w_d\cdot dist_{ij,d}^2\\
								= & (\bm{x}_i-\bm{x}_j)^{\textrm{T}}\mathbf{W}(\bm{x}_i-\bm{x}_j)
							\end{align*}
							\item 若属性之间相关，则$\mathbf{W}$被替换为半正定对称矩阵$\mathbf{M}$，得到``马氏距离''为\[\mathrm{dist}_{\mathrm{mah}}^2=(\bm{x}_i-\bm{x}_j)^{\textrm{T}}\mathbf{M}(\bm{x}_i-\bm{x}_j)=||\bm{x}_i-\bm{x}_j||_\mathbf{M}^2\]度量学习就是对度量矩阵$\mathbf{M}$进行学习。
						\end{enumerate}
						\item 近邻成分分析（NCA）：对于任意样本$\bm{x}_j$，其对$\bm{x}_i$分类结果影响的概率为\[p_{ij}=\frac{\exp\big(-||\bm{x}_i-\bm{x}_j||_{\mathbf{M}}^2\big)}{\sum_{l}^{}\exp\big(-||\bm{x}_i-\bm{x}_l||_{\mathbf{M}}^2\big)}\]以留一法正确率最大化为目标，$\bm{x}_i$留一法正确率为\[p_i=\sum_{j\in\Omega_i}^{}p_{ij}\]其中$\Omega_i$表示与$\bm{x}_i$属于相同类别的样本的下标集合。整个样本集的留一法正确率：\[\sum_{i=1}^{m}p_i=\sum_{i=1}^{m}\sum_{j\in\Omega_i}^{}p_{ij}\]考虑$\mathbf{M}=\mathbf{PP}^{\textrm{T}}$，则NCA的优化目标为\[\min\limits_{\mathbf{P}}\quad1-\sum_{i=1}^{m}\sum_{j\in\Omega_i}^{}\frac{\exp\big(-||\mathbf{P}^{\textrm{T}}\bm{x}_i-\mathbf{P}^{\textrm{T}}\bm{x}_j||_2^2\big)}{\sum_{l}^{}\exp\big(-||\mathbf{P}^{\textrm{T}}\bm{x}_i-\mathbf{P}^{\textrm{T}}\bm{x}_l||_2^2\big)}\]
						\item 若已知某些样本相似、某些样本不相似，定义``必连''约束集合$\mathcal{M}$与``勿连''约束集合$\mathcal{C}$。其中$(\bm{x}_i,\bm{x}_j)\in\mathcal{M}$表示$\bm{x}_i$与$\bm{x}_j$相似，$(\bm{x}_i,\bm{x}_k)\in\mathcal{C}$表示$\bm{x}_i$与$\bm{x}_k$不相似。构造如下凸优化问题\begin{align*}
						\min\limits_{\mathbf{M}}\quad & \sum_{(\bm{x}_i,\bm{x}_j)\in\mathcal{M}}^{}||\bm{x}_i-\bm{x}_j||_{\mathbf{M}}^2 \\
						\mathrm{s.t.}\quad & \sum_{(\bm{x}_i,\bm{x}_k)\in\mathcal{C}}^{}||\bm{x}_i-\bm{x}_k||_{\mathbf{M}}\ge1\\
						& \mathbf{M}\succeq0
						\end{align*}
					\end{itemize}
			\section{特征选择与稀疏学习}
				\subsection{子集搜索与评价}
					\begin{itemize}
						\item 特征：属性；相关特征：对当前学习任务有用的属性；无关特征：对当前学习任务用处不大的属性；特征选择：从给定的特征集合中选择出相关特征子集的过程。
						\item 特征选择与降维是处理高维数据的两大主流技术。
						\item 冗余特征：包含的信息能从其它特征推演出来的一类特征。
						\item 没有任何领域知识作为先验假设 $\rightarrow$ 遍历所有的子集 $\rightarrow$ 组合爆炸
						\item 两个关键环节\begin{enumerate}
							\item 子集搜索：贪心策略（仅考虑使本轮选定集最优）\begin{itemize}
								\item 前向搜索：将每个特征看作一个候选子集，对$d$个候选单特征子集进行评价，将最优的单特征添加进去，然后逐渐从剩余的候选单特征子集中择优添加，直到添加后的子集不如上一轮选定集，此时将上一轮选定的集合作为特征选择结果。
								\item 后向搜索：从完整的特征集合开始逐渐去掉特征。
								\item 双向搜索：每一轮逐渐增加选定相关特征同时减少无关特征。
							\end{itemize}
							\item 子集评价：基于信息增益。假定样本属性均为离散型。对属性子集$A$，假定根据其取值将$D$分成了$V$个子集$\{D^1,D^2,\dots,D^V\}$，则属性子集$A$的信息增益为\[\mathrm{Gain}(A)=\mathrm{Ent}(D)-\sum_{v=1}^{V}\frac{|D^v|}{|D|}\mathrm{Ent}(D^v)\]\[\mathrm{Ent}(D)=-\sum_{k=1}^{|\mathcal{Y}|}p_k\log_2p_k\]信息增益越大，则该特征子集包含的有助于分类的信息越多。扩展：只要能估算样本标记信息对$D$的真实划分与特征子集对$D$的划分这两者的差异即可。
						\end{enumerate}
					\end{itemize}
				\subsection{过滤式选择}
					\begin{itemize}
						\item 先对数据集进行特征选择，再训练学习器。
						\item Relief方法：利用``相关统计量''度量特征的重要性。\begin{enumerate}
							\item 对于每个示例$\bm{x}_i$，先在$\bm{x}_i$的同类样本中寻找最近邻$\bm{x}_{i,\mathrm{nh}}$，即为``猜中近邻''。
							\item 再从$\bm{x}_i$的异类样本中寻找其最近邻$\bm{x}_{i,\mathrm{nm}}$，即为``猜错近邻''。
							\item 相关统计量在属性$j$的分量为\[\delta^j=\sum_{i}^{}-\mathrm{diff}(x_i^j,x_{i,\mathrm{nh}}^j)^2+\mathrm{diff}(x_i^j,x_{i,\mathrm{nm}}^j)^2\]若属性$j$为离散型，则$\mathrm{diff}(x_a^j,x_b^j)$为1或0，否则为$|x_a^j-x_b^j|$，此时每个属性上的取值需规范化到$[0,1]$区间。
							\item Relief在数据集上多次采样并相应地估计相关统计量。
						\end{enumerate}
						\item Relief-F方法：处理多分类问题。设$p_l$为第$l$类样本在数据集$D$中所占的比例。\[\delta^j=\sum_{i}^{}-\mathrm{diff}(x_i^j,x_{i,\mathrm{nh}}^j)^2+\sum_{l\neq k}^{}\big(p_l\times \mathrm{diff}(x_i^j,x_{i,\mathrm{i,l,\mathrm{nm}}}^j)^2\big)\]
					\end{itemize}
				\subsection{包裹式选择}
					\begin{itemize}
						\item 包裹式特征选择：直接把最终将要使用的学习器的性能作为特征子集的评价准则。计算开销很大，但从最终学习器性能来看比过滤式特征选择更好。
						\item LVW算法：在Las Vegas方法框架下使用随机策略进行子集搜索，以最终分类器的误差为特征子集评价准则。\begin{framed}
							拉斯维加斯算法不会得到不正确的解，一旦用拉斯维加斯算法找到一个解，那么这个解一定就是正确的解，但有时用拉斯维加斯算法找不到解。而蒙特卡洛方法一定会给出解，虽然给出的解未必满足要求。若无时间限制，则两者都能给出满足要求的解。
						\end{framed}
						\begin{algorithm}
							\caption{LLE算法}
							\begin{algorithmic}[1]
								\REQUIRE 数据集$D$、特征集$A$、学习算法$\mathfrak{L}$、停止条件控制参数$T$
								\ENSURE 特征子集$A^*$
								\STATE $E=\infty$; \\
								\STATE $d=|A|$; \\
								\STATE $A^*=A$; \\
								...
								\WHILE{$t<T$}
									\STATE 随机产生特征子集$A'$; \\
									\STATE $d'=|A'|$; \\
									\STATE $E'=\mathrm{CrossValidation}(\mathfrak{L}(D^{A'}))$; \\
						...
						\end{algorithm}
					\end{itemize}
				\subsection{嵌入式选择与L$_1$正则化}
					\begin{itemize}
						\item 嵌入式特征选择：将特征选择过程与学习器训练过程融合，在同一优化过程中完成。
						\item 线性回归模型中，使用$\mathrm{L}_1$范数正则化，比$\mathrm{L}_2$范数正则化更易于获得稀疏解，即有更多的分量为零。此时的优化目标称为LASSO：\[\min\limits_{\bm{w}}\sum_{i=1}^{m}(y_i-\bm{w}^{\textrm{T}}\bm{x}_i)^2+\lambda||\bm{w}||_1\]如此求解$\mathrm{L}_1$范数正则化的结果是得到了仅采用一部分初始特征的模型，即为一种嵌入式特征选择方法。
						\item 可用近端梯度下降（PGD）求解。
					\end{itemize}
				\subsection{稀疏表示与字典学习}
					\begin{itemize}
						\item 稀疏矩阵：存在很多零元素的矩阵（不一定是整行整列）
						\item 字典学习/稀疏编码：为普通稠密表达的样本找到合适的字典，将样本转化为合适的稀疏表示形式，简化学习任务，降低模型复杂度。
						\item 字典学习最简形式：\[\min\limits_{\mathbf{B},\bm{\alpha}_i}\sum_{i=1}^{m}||\bm{x}_i-\mathbf{B}\bm{\alpha}_i||_2^2+\lambda\sum_{i=1}^{m}||\bm{\alpha}_i||_1\]
						其中$\textbf{B}\in\mathbb{R}^{d\times k}$为字典矩阵，$k$为字典词汇量，$\bm{\alpha}_i\in\mathbb{R}^k$为样本$\bm{x}_i\in\mathbb{R}^d$的稀疏表示。第一项使$\bm{\alpha}_i$更好地重构$\bm{x}_i$，第二项使$\bm{\alpha}_i$尽量稀疏。
						\item 求解方法：变量交替优化\begin{enumerate}
							\item 先固定字典$\mathbf{B}$，按分量展开为\[\min\limits_{\bm{\alpha}_i}||\bm{x}_i-\mathbf{B}\bm{\alpha}_i||_2^2+\lambda||\bm{\alpha}_i||_1\]参照LASSO的解法求解。
							\item 以$\bm{\alpha}_i$为初值来更新字典$\mathbf{B}$。目标函数化为\[\min\limits_{\mathbf{B}}||\mathbf{X}-\mathbf{BA}||_{F}^2\]其中$\mathbf{X}=(\bm{x}_1,\bm{x}_2,\dots,\bm{x}_m)\in\mathbb{R}^{d\times m}$，$A=(\bm{\alpha}_1,\bm{\alpha}_2,\dots,\bm{\alpha}_m)\in\mathbb{R}^{k\times m}$，$||\cdot||_F$是Frobenius范数。可用基于逐列更新策略的KSVD求解。
						\end{enumerate}
					\end{itemize}
				\subsection{压缩感知}
					\begin{itemize}
						\item 压缩感知模型：假定有长度为$m$的离散信号$\bm{x}$，不妨假定以远小于奎奈斯特采样定理要求的采样率进行采样，得到长度为$n$的采样后信号$\bm{y}$，有$n\ll m$且$\bm{y}=\bm{\Phi x}$，其中$\Phi\in\mathbb{R}^{n\times m}$是对信号$\bm{x}$的测量矩阵。由$\bm{y}$和$\bm{\Phi}$反推$\bm{x}$，可令$\bm{x}=\bm{\Psi s}$，其中$\bm{s}$具有稀疏性。此时$\bm{\Psi}$为稀疏基，$\mathbf{A}=\bm{\Phi\Psi}$作用类似于字典。
						\item 矩阵补全技术\begin{align*}
							\min\limits_{\mathbf{X}}\quad & \mathrm{rank}(\mathbf{X}) \\
							\mathrm{s.t.}\quad & (\mathbf{X})_{ij}=(\mathbf{A})_{ij},\quad (i,j)\in\Omega
						\end{align*}
						其中$\mathbf{X}$表示需要恢复的稀疏信号。$\Omega$是$\mathbf{A}$中有数据元素的下标集合。这是一个NP难问题。可转化最小化矩阵核范数再通过半正定规划求解。
					\end{itemize}
			\section{计算学习理论}
			\section{半监督学习}
